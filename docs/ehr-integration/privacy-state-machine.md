...
| **INV-8** | **No PHI at rest or in logs.** Logs carry field ids, resource types, FHIRPaths, confidence, hashes, actors, timestamps — never values from the chart. Structured `Patient.name`/`identifier`/`address`/`telecom` are excluded and `birthDate` is consumed into age then discarded; **free-text** values are treated as tainted and pass a hardened, fail-closed leak-guard (B1). | Never-extract tests; `TestLeakGuardHostile` (guard must fire on PHI in `Condition.code.text` / `dosageInstruction.text`, case/format variants, contained + RelatedPerson names, and fail closed when no Patient basis exists); privacy-log test asserts no chart value recorded. | **Implemented + tested** (structured **and** free-text). Encrypted-at-rest / bundle-purge-on-commit remain deferred with INV-3. |
...
Deferred (tracked, not claimed as present): the typed `commit_profile`/committed-profile object and per-field re-confirmation gate (INV-3); the `DeidentifiedPredicates` egress gateway (INV-4); `promote()` mode transitions and the enrollment path (INV-5); encrypted-at-rest bundle retention and purge-on-commit (INV-8 tail); the real SMART OAuth launch with read-only scope assertion (INV-7 tail).
