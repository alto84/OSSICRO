...
## Addendum 2026-07-09d — Rapid-access vision (planned pages)

_New architecture pages implied by the rapid-access expanded plan ([`docs/rapid-access-vision-and-plan.md`](../docs/rapid-access-vision-and-plan.md)); to be authored._

- `wiki/05-ossicro-system/rapid-access-conductor.md` — The per-patient case state machine and parallel-pathway orchestration doctrine: from one intake, run Mode A / Mode B / expanded access as warm parallel tracks; track every counterparty clock (312.53(a) sponsor response, manufacturer LOA, IRB cadence, FDA 30-day and 15-working-day windows); execute the fallback flip so a counterparty no costs days not months. Route election is a logged human decision; every submission stays gate-red to the physician.
- `wiki/05-ossicro-system/fact-spine-and-parallel-drafting.md` — The canonical StudyFacts record as the single permitted source of every document fact, the parallel drafting-swarm architecture (one bounded-context subagent per registry document), and why cross-document consistency holds by construction rather than by post-hoc checking. Includes the [[MISSING]]-to-red invariant and the provenance-triple-per-span contract.
- `wiki/05-ossicro-system/fill-only-the-blanks-contract.md` — The generation invariant: the drafting agent emits only typed field values for enumerated placeholders; the engine interpolates into human-vetted templates; a deterministic template-diff rejects any non-placeholder byte delta; verbatim-required passages (50.25(c), 1572 Field 9) are SHA-256-locked. The structural basis of the anti-slop guarantee.
- `wiki/05-ossicro-system/anti-slop-writing-quality-layer.md` — The four-layer voice architecture: the deterministic register linter with the versioned banned-construction registry, the per-category voice-validation checking agent, the span-scoped flag-and-rewrite loop, and the golden-corpus regression harness with per-rule precision/recall. Includes the banned-construction list, the consent exculpatory-language gate (50.20), and the false-positive-control discipline for legitimate regulatory register.
- `wiki/05-ossicro-system/red-resolver-intake-loop.md` — The ledger-driven convergence-to-zero-reds interview: how the completeness ledger's resolving questions drive a bounded, dependency-ordered intake that recomputes after every answer, converts chart-datum reds into SMART-on-FHIR fetches and counterparty-artifact reds into tracked outbound requests, and defines done as a computed state. The agent supplies data; it never grades or dispositions a red.
- `wiki/05-ossicro-system/route-classifier-and-hard-stops.md` — The bridge from matching to paperwork: the triage agent that drafts a rapid-access route recommendation from the profile and adjudicated candidate ledger, surfacing the four software-unfixable hard stops (central-IRB access/fee, research-liability indemnification, NCD 310.1 billing qualification, site-readiness against the schedule of assessments) up front as green/amber/red gates before any dossier drafting begins.
- `wiki/04-coordination/manufacturer-front-door-registry.md` — The maintained machine-readable directory of manufacturers' sanctioned intake points (published §561A expanded-access policies, IIS/grants portals, medical-information contacts) with per-record source URL, retrieval date, and hash; the route router that resolves a drug to its manufacturer's front door with the Medical-Affairs-only hard rail; and the freshness/diff discipline. Proposed as an open-source public-good asset.
- `wiki/04-coordination/say-yes-packages-and-checklist-join.md` — The counterparty-facing export doctrine: route-specific sponsor-native packs (EA LOA-request, IIS, TMF-Reference-Model activation binder) with the cross-reference index and hash manifest, plus the Checklist Join tool that ingests a sponsor's own feasibility questionnaire and maps each line-item to an OSSICRO artifact/citation/signer with human-confirmed mappings. Standardize the package, not the pipe.
- `wiki/05-ossicro-system/sign-and-ship-cockpit.md` — The terminal assembly and gate-discharge stage: the hash-chained package manifest (independently verifiable requirement-to-signer), the single-pass review surface with deterministic spans collapsed and inferred spans foregrounded, the amber signature-routing queue, Part 11 sign-off capture (11.50-11.300), and deterministic official-form PDF fill unlocked only when the state machine permits. Submission remains an explicit human act; GateViolation enforces.
- `wiki/05-ossicro-system/days-to-drug-clock-model.md` — The critical-path measurement layer: per-route decomposition of time-to-drug into physician-side intervals (driven toward zero by reusable artifacts), cited statutory clocks, and observed counterparty intervals shown as elapsed time only with no fabricated predictions. The observability that proves 'heroic access has become routine' with medians rather than assertion; changes no document and sends nothing.
- `wiki/06-personas/one-visit-feasibility-verdict.md` — The physician adoption-trigger flow documented as a persona-facing capability: one session in, route triage + the four hard stops named as the first output + honest hours/cost table + complete draft dossier out. Anchors the hcp-physician promise that a physician who discovers the insurance or billing gap after assembling the package has been failed by the tool.
- `wiki/05-ossicro-system/golden-package-harness.md` — The regression-eval infrastructure: synthetic study fixtures paired with human-verified gold packages, replayed on every template/rule/prompt/model-version change, reporting form-field exactness, provenance coverage, ledger-grade agreement, register-lint pass rate, and zero-gate-regression. OSSICRO's own credibility file under the FDA 2025 AI-credibility draft guidance, defending the low-influence/low-consequence COU with data.
